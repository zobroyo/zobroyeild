if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("Zobro Yeild is already running!", 0)
    return
end
...
Scale.Name = rando…f everyClipboard then
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Zobro Yeild Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Zobro Yeild support server.'}
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, Zobro Yeild may have issues or features that are not functioning correctly.") end
end)
